...
>
>Azure functions could also be implemented in an [Azure Logic App](https://azure.microsoft.com/en-us/services/logic-apps/). A comparison between durable functions and Logic apps can be found [here](https://docs.microsoft.com/en-us/azure/azure-functions/functions-compare-logic-apps-ms-flow-webjobs).
>
...
## DELETE THIS
1. ArticlesIngestTrigger function triggered from a CosmosDB insert. This insert notification then passes along document id's when the incoming item is an article and pushes the Cosmos document id onto an Azure Service Bus queue for translation.
2. The TranslationQueueFunction function is triggered from the first queue. This detects the language of the incoming text (title and body) and then, if not in the predetermined language, translates it to the desired language. Next it processes the text to find key phrases, sentiment value, and known entities. This function then passes along the Cosmos document id to the next queue which triggers the next function. 
3. The OCRQueueTrigger is next function in line. It looks at images and (1) pulls the text from the image using OCR and (2) uses computer vision to determine what is in the image (objects). This function then passes along the Cosmos document id to the next queue which triggers the next function.
4. The FaceAPIQueueTrigger function looks again through the images and uses the Face API service to determine (1) number of people (2) gender and age of those found. This function then passes along the Cosmos document id to the next queue which triggers the next function.
5. The InspectionQueueTrigger is the last funciton in this configuration. It reads through all of the processed records for an associated article and collects some information and writes a record back to Cosmos DB. This step, in a real world scenario, would likely trigger some other notification event (email, service bus, other), but given this is a demo, that exercise is left to the reader. 
